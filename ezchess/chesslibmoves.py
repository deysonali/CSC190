...
def GetPieceLegalMoves(board,position):
	...
	if isKingInCheck(board,player):
		for i in moves:
			...
			if KingCheck(testboard,player) == False:
				accum = accum + [i]
		return accum	
...
def GetMoves(board,position):
	...
	if board[position] == 12: #white bishop
		...
		right = [i for i in range(0,57,8)]
		left = [i for i in range(7,64,8)]
		topr = position
		topl = position
		botr = position
		botl = position
		...
		while True:
                        ...
                        if botl in bpos:
                                break
		for i in range(botl,position,7):
			accum = accum + [i]
		for j in range(botr,position,9):
			accum = accum + [j]
		for k in range(topl,position,-9):
			accum = accum + [k]
		for l in range(topr,position,-7):
			accum = accum + [l]
